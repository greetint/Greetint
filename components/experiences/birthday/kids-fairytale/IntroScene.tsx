...
export function IntroScene({ childName, onComplete }: IntroSceneProps) {
  ...
  return (
    ...
    >
      ...
      {/* 2. World Reveal Layer (Island & Castle) */}
      <motion.div 
      ...
      >
        <motion.div 
          ...
          className="absolute -top-10 w-[80%] max-w-md opacity-90 drop-shadow-[0_10px_30px_rgba(0,0,0,0.6)]"
        >
          <img 
        ...
        <div className="absolute bottom-0 w-full opacity-95">
          <img 
      ...
      <AnimatePresence>
        {stage !== 'curtain' && (
          <motion.div
            ...
            className="absolute top-8 sm:top-10 z-20 px-6 max-w-lg w-full flex flex-col items-center space-y-2 pointer-events-none"
          >
            ...
            <h1 className="text-2xl sm:text-3xl font-serif font-black text-amber-200 drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)] leading-tight">
              Имало едно време едно вълшебно царство...
            </h1>
            <div className="text-3xl sm:text-5xl font-serif font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-pink-300 to-amber-200 drop-shadow-[0_4px_16px_rgba(251,191,36,0.8)] py-1">
              ✦ {childName} ✦
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {stage !== 'curtain' && (
          <motion.div
            ...
            className="absolute bottom-16 sm:bottom-20 z-30 flex flex-col items-center cursor-pointer group"
          ...
          >
            ...
            <motion.div
              ...
              whileTap={{ scale: 0.85 }}
              className="relative p-5 sm:p-6 bg-gradient-to-br from-yellow-300 via-amber-400 to-orange-500 rounded-full shadow-[0_0_40px_rgba(251,191,36,0.9)] border-4 border-white flex items-center justify-center cursor-pointer"
            >
              <img 
                src="/images/birthday/kids_fairytale/stage1/key.png" 
                alt="Golden Key" 
                className="w-14 h-14 sm:w-20 sm:h-20 object-contain drop-shadow-md group-hover:scale-110 transition-transform duration-300"
              />
            ...
            <motion.div
              ...
              className="mt-3 bg-indigo-950/80 backdrop-blur-md px-5 py-2 rounded-2xl border border-amber-300/60 shadow-xl text-amber-200 text-xs sm:text-sm font-bold tracking-wide pointer-events-none"
            >
      ...
      <div className="absolute inset-0 z-40 pointer-events-none overflow-hidden flex items-center justify-between">
        ...
        >
          <img 
            ...
            className="absolute -left-20 w-[150%] h-full object-cover opacity-95 filter brightness-105 drop-shadow-2xl"
          />
        </motion.div>
        ...
        >
          <img 
            ...
            className="absolute -right-20 w-[150%] h-full object-cover opacity-95 filter brightness-105 drop-shadow-2xl scale-x-[-1]"
          />
        </motion.div>
      </div>

      {/* 6. Climax Flash / Fade to White Effect */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: stage === 'unlocking' ? [0, 0.3, 1] : 0 }}
        transition={{ duration: 1.2, times: [0, 0.6, 1] }}
        className="absolute inset-0 z-50 bg-gradient-to-tr from-amber-100 via-white to-yellow-200 pointer-events-none"
      />
    </motion.div>
  );
}
