...
export function CakeScene({ childAge, childName, isMuted = false, onComplete }: CakeSceneProps) {
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen overflow-hidden bg-black flex items-center justify-center select-none z-50" onClick={subStage < 5 ? handleInteract : undefined}>
      <audio ref={audioRef} src={audioSrc} preload="auto" onEnded={() => setAudioEnded(true)} />
      <div className="absolute inset-0 z-0 w-full h-full">
        <VideoPlayerManager
          src={currentSrc}
          onEnded={() => { if (subStage === 6) onComplete(childWish || 'Вълшебно желание'); }}
        ...
        <div className="absolute inset-0 z-[50] flex flex-col items-center justify-center pointer-events-auto bg-black/30 backdrop-blur-sm p-4">
          <div className="max-w-md w-full bg-amber-950/80 border-2 border-amber-300 p-6 rounded-3xl text-center space-y-4 shadow-2xl backdrop-blur-xl">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => setChildWish('Вълшебно детско пожелание')} className="w-full py-3 px-6 rounded-2xl bg-amber-500 text-slate-950 font-serif font-bold text-sm shadow-lg cursor-pointer">Намисли желание</button>
              <button onClick={() => setSubStage(6)} className="w-full py-3 px-6 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 text-white font-serif font-bold text-sm shadow-lg flex items-center justify-center gap-2 cursor-pointer">
