...
export function ScratchStage({ 
  ...
  const checkScratchPercentage = () => {
    ...
    // Увеличено на 80%, за да се изисква обстойно изтриване на по-голяма площ преди авто-премахване
    if (percentage > 80) {
      setIsScratched(true);
    }
  };
  ...
  return (
    <div className="relative w-screen h-screen flex flex-col items-center justify-between overflow-hidden select-none p-6 sm:p-16">
      {/* ФОН НА ЦЯЛ ЕКРАН */}
      <div 
      ...
      <motion.div
        ...
        className="relative w-full h-full max-w-4xl flex flex-col justify-between items-center z-10 py-6 sm:py-10 text-center"
      >
        <div className="space-y-3">
          <span className="text-xs sm:text-sm uppercase tracking-[0.4em] text-[#958679] font-sans font-bold block">
          ...
          <div className="w-24 h-[1px] bg-[#958679]/50 mx-auto mt-2" />
        </div>

        {/* ЗОНА ЗА ТРИЕНЕ */}
        <div 
          ref={containerRef}
          className="relative w-full max-w-xl h-44 sm:h-56 my-auto flex items-center justify-center rounded-2xl overflow-hidden border border-[#958679]/40 shadow-[0_20px_50px_rgba(31,26,23,0.15)] bg-[#FEFEFD]/70 backdrop-blur-sm p-6 sm:p-10"
        >
          {/* ТЕКСТ */}
          <div className="w-full max-h-full overflow-y-auto px-2 z-0">
            <motion.p 
              ...
              className={`font-serif italic text-base sm:text-xl text-[#1F1A17] leading-relaxed text-center px-2 drop-shadow-[0_2px_10px_rgba(219,206,179,0.5)] break-words transition-opacity duration-300 ${
            ...
            {isScratched ? (
              ...
              >
                <button
                  ...
                  className="bg-[#1F1A17] text-[#FEFEFD] px-10 py-4 text-xs uppercase tracking-[0.3em] font-bold rounded-xl shadow-[0_15px_30px_rgba(31,26,23,0.25)] font-sans hover:bg-[#958679] transition duration-300 w-full"
                >
              ...
              <motion.span 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="text-xs uppercase tracking-[0.25em] text-[#635E57] font-sans font-bold italic block drop-shadow-sm"
              >
                ✦ Плъсни с пръст или мишка, за да изтриеш фолиото ✦
              </motion.span>
            )}
          </AnimatePresence>
        </div>
