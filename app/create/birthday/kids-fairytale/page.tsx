...
export default function KidsFairytaleCreatePage() {
  ...
  const [fd, setFd] = useState({
    childName: t('kidsFairytale.sandboxPage.defaultChildName'),
    childAge: '6',
    senderName: t('kidsFairytale.sandboxPage.defaultSenderName'),
    personalMessage: t('kidsFairytale.sandboxPage.defaultPersonalMessage'),
    favoriteAnimal: t('kidsFairytale.sandboxPage.defaultFavoriteAnimal'),
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const link = `${window.location.origin}/kids-fairytale?id=${id}&lang=${lang}`;
      ...
      <div className="max-w-xl w-full bg-white/95 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-pink-200 space-y-6">
        <div className="flex justify-between items-center border-b border-pink-100 pb-4">
          <Link href="/create/birthday/select-style" className="text-xs font-bold uppercase tracking-widest text-[#958679] hover:text-[#2C241D] transition flex items-center gap-1">
            <ArrowLeft className="w-4 h-4" /> {t('kidsFairytale.createForm.backLink')}
          </Link>
          <Logo variant="icon-only" height={40} />
        </div>

        <div className="text-center space-y-2">
          <span className="text-[10px] uppercase tracking-[0.25em] bg-pink-100 text-pink-700 px-3 py-1 rounded-full font-bold">{t('kidsFairytale.createForm.sandboxBadge')}</span>
          <h1 className="text-2xl font-serif font-bold text-[#2C241D]">{t('kidsFairytale.createForm.title')}</h1>
        </div>
        ...
        {!createdLink ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-[#2C241D]/70 block mb-1">{t('kidsFairytale.createForm.labels.name')}</label>
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-[#2C241D]/70 block mb-1">{t('kidsFairytale.createForm.labels.age')}</label>
            ...
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-[#2C241D]/70 block mb-1">{t('kidsFairytale.createForm.labels.sender')}</label>
            ...
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-[#2C241D]/70 block mb-1">{t('kidsFairytale.createForm.labels.message')}</label>
            ...
            <button type="submit" className="w-full bg-gradient-to-r from-pink-500 to-rose-500 text-white py-4 rounded-2xl text-xs font-bold uppercase tracking-[0.25em] shadow-lg hover:opacity-90 transition flex items-center justify-center gap-2 cursor-pointer">
              <Sparkles className="w-4 h-4" /> {t('kidsFairytale.createForm.submitButton')}
            </button>
          </form>
        ) : (
          <div className="space-y-6 text-center py-6">
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-emerald-800 text-xs font-medium">
              {t('kidsFairytale.createForm.successMessage')}
            </div>
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest text-[#2C241D]/60 font-bold block">{t('kidsFairytale.createForm.linkLabel')}</label>
            ...
            <div className="flex gap-4">
              <a href={createdLink} target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-pink-500 to-rose-500 text-white py-3.5 rounded-2xl text-xs font-bold uppercase tracking-widest shadow-md text-center block">
                {t('kidsFairytale.createForm.openLink')}
              </a>
              <button onClick={() => setCreatedLink(null)} className="px-5 bg-gray-200 text-[#2C241D] py-3.5 rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-gray-300 transition">
                {t('kidsFairytale.createForm.newButton')}
              </button>
            </div>
          </div>
        )}
